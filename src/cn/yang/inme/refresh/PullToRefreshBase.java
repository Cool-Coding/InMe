...
import cn.yang.inme.refresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
     ...
     */
      public interface OnRefreshListener<V extends View> {
         ...
         * 当过滤时传入序号
         */
        void onFilter(final int orderIndex,final boolean tuan,final boolean quan,final boolean onLine);
    }
    ...
    private Logger log = Logger.getLogger(PullToRefreshBase.class.getName());
                    ...
                    if (isPullRefreshEnabled() && isReadyForPullDown()) {
                        ...
                        if (mIsHandledTouchEvent) {
                            try {
                                ...
                                e.printStackTrace();
                                log.error("pullToRefreshBase",e.getCause());
                            }
                        }
     ...
     */
    protected void addRefreshableView(final Context context, T refreshableView) {
        ...
        //添加过滤窗口
        final ImageView filter=new ImageView(context);
        final FrameLayout.LayoutParams params1=new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT,FrameLayout.LayoutParams.WRAP_CONTENT);
        params1.gravity= Gravity.BOTTOM|Gravity.RIGHT;
        params1.bottomMargin=50;
        params1.rightMargin=20;
        filter.setBackgroundResource(R.drawable.ic_action_filter);
        filter.setLayoutParams(params1);

        OnClickListener onClickListener=new OnClickListener() {
            //团购
            String[] tuangou_filter_text=new String[]{"距我近优先","价格低优先","价格高优先","购买人数优先"};
            int[] tuangou_filter_image=new int[]{R.drawable.up_arrow,R.drawable.up_arrow,R.drawable.down_arrow,R.drawable.down_arrow};
            int[] tuangou_sortIndex=new int[]{7,2,3,4};
            //店铺
            String[] shop_filter_text=new String[]{"距我近优先","评价高优先","点评数量多优先","环境评价高优先","人均价格低优先"};
            int[]    shop_filter_image=new int[]{R.drawable.up_arrow,R.drawable.down_arrow,R.drawable.down_arrow,R.drawable.down_arrow,R.drawable.up_arrow};
            int[]    shop_sortIndex=new int[]{7,2,6,4,8};

            private int kind=0;
            private LayoutParams params1 =new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            private LayoutParams params2 =new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            private PopupWindow popupWindow;
            private int oven=1;
            @Override
            public void onClick(View v) {
                if(popupWindow!=null){
                    if (oven++%2==0)popupWindow.dismiss();
                    else popupWindow.showAtLocation(v, Gravity.RIGHT | Gravity.BOTTOM, v.getWidth() + 22, Constants.footerHeight+3);
                    return;
                }

                int count;
                if ("AroundTuangouRefresh".equals(PullToRefreshBase.this.getClassName())){
                    kind = 1;
                }
                else if("AroundShopRefresh".equals(PullToRefreshBase.this.getClassName())){
                    kind=2;
                }
                ...
                if (kind==1) {
                    ...
                    count = tuangou_filter_text.length;
                    for(int i=0;i<count;i++) {
                        RadioButton radioButton=new RadioButton(context);
                        radioGroup.addView(radioButton);

                        radioButton.setLayoutParams(params1);
                        Drawable drawable=getResources().getDrawable(tuangou_filter_image[i]);
                        radioButton.setCompoundDrawablesWithIntrinsicBounds(null,null,drawable,null);
                        radioButton.setId(i);
                        radioButton.setText(tuangou_filter_text[i]);
                    }
                    radioGroup.check(filterIndex);
                    radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                        @Override
                        public void onCheckedChanged(RadioGroup group, int checkedId) {
                            filterIndex = checkedId;
                            if (mRefreshListener != null) mRefreshListener.onFilter(tuangou_sortIndex[filterIndex],false,false,false);
                            popupWindow.dismiss();
                        }
                    });
                    ...
                    int themeColor=Integer.valueOf(PropertiesUtil.instance().read(Constants.SET_THEME_COLOR));

                    LinearLayout contentView = new LinearLayout(context);
                    contentView.setOrientation(VERTICAL);

                    LinearLayout oneLine = new LinearLayout(context);
                    oneLine.setOrientation(HORIZONTAL);
                    oneLine.setGravity(Gravity.RIGHT);
                    oneLine.setPadding(0,30,30,0);

                    BorderTextView sureBt=new BorderTextView(context);
                    sureBt.setBackgroundColor(Color.TRANSPARENT);
                    sureBt.setBottomBorder(themeColor,2);
                    sureBt.setText("确定");
                    sureBt.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
                    oneLine.addView(sureBt);

                    LinearLayout twoLine = new LinearLayout(context);
                    twoLine.setOrientation(HORIZONTAL);

                    RadioGroup radioGroup=new RadioGroup(context);
                    radioGroup.setLayoutParams(params2);
                    radioGroup.setOrientation(VERTICAL);
                    count = shop_filter_text.length;
                    for(int i=0;i<count;i++) {
                        RadioButton radioButton=new RadioButton(context);
                        radioGroup.addView(radioButton);

                        radioButton.setId(i);
                        radioButton.setText(shop_filter_text[i]);
                    }
                    radioGroup.check(filterIndex);
                    radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                        @Override
                    ...
                    twoLine.addView(radioGroup);

                    BorderLinearLayout checkLayout = new BorderLinearLayout(context);
                    checkLayout.setOrientation(VERTICAL);
                    checkLayout.setGravity(Gravity.CENTER);
                    checkLayout.setBackgroundColor(Color.TRANSPARENT);
                    checkLayout.setLeftBorder(Color.GRAY, 1);
                    checkLayout.setPadding(15, 0, 15, 0);
                    checkLayout.setLayoutParams(params1);
                    checkLayout.invalidate();

                    final CheckBox checkBox1 = new CheckBox(context);
                    checkBox1.setText("团购");
                    checkBox1.setChecked(tuan);
                    checkLayout.addView(checkBox1);

                    final CheckBox checkBox2 = new CheckBox(context);
                    checkBox2.setText("优惠券");
                    checkBox2.setChecked(quan);
                    checkLayout.addView(checkBox2);

                    final CheckBox checkBox3 = new CheckBox(context);
                    checkBox3.setText("在线预订");
                    checkBox3.setChecked(onLine);
                    checkLayout.addView(checkBox3);

                    twoLine.addView(checkLayout);

                    sureBt.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if (checkBox1.isChecked())tuan=true;
                            else tuan=false;
                            if (checkBox2.isChecked())quan=true;
                            else quan=false;
                            if (checkBox3.isChecked())onLine=true;
                            else onLine=false;

                            if (mRefreshListener != null) mRefreshListener.onFilter(shop_sortIndex[filterIndex],tuan,quan,onLine);
                            popupWindow.dismiss();
                        }
                    });
                    contentView.addView(oneLine);
                    contentView.addView(twoLine);
...
/*                ShapeDrawable shapeDrawable = new ShapeDrawable();
                ...
                popupWindow.setBackgroundDrawable(getResources().getDrawable(R.drawable.filterpopupwindow));
                popupWindow.setAnimationStyle(R.style.popwin_anim_style);
                popupWindow.setOutsideTouchable(true);
                popupWindow.getBackground().setAlpha(200);
                //popupWindow.setFocusable(true);//加了这行，ListView才能响应单击事件
                popupWindow.showAtLocation(v, Gravity.RIGHT | Gravity.BOTTOM, v.getWidth() + 22, Constants.footerHeight+3);
                oven++;
            }
        };
        ...
        mRefreshableViewWrapper.addView(filter);
